...
import com.entity.NetPackage;
import com.entity.ServicePackage;
import com.entity.SuperPackage;
import com.entity.TalkPackage;
import com.info.ConsumInfo;
import com.info.MobileCard;
import com.info.Scene;
...
//    private static final ServicePackage NetPackage = null;
    ...
    Scene scene5 = new Scene("上网", 1024, "和老婆语音通话，通宵一晚，使用流量1GB");


    //用户数据初始化
    public void initData() {
        // 初始化三张手机卡,并添加进集合
        MobileCard card1 = new MobileCard("13912345678", "张三", "1234",
                new TalkPackage(), 68, 100.0, 200, 20, 300, 58.0);// 话痨套餐
        MobileCard card2 = new MobileCard("13979387058", "发阿萨德", "12345",
                new NetPackage(), 78, 22.0, 100, 10, 3000, 68.0);// 网虫套餐
        MobileCard card3 = new MobileCard("13979387059", "当阿萨德", "123456",
                new SuperPackage(), 88, 12.0, 200, 20, 500, 78.0);// 超人套餐
        map.put("13912345678", card1);
...
/*以“139”开头，后面拼接8位随机数
使用java.util.Random()类生成指定位数的随机数*/
    ...
    public void showAmountDetail(String yourNumber) {
        ...
        System.out.println("套餐资费：" + map.get(yourNumber).setPackage.price + "元");
...
// 套餐余量查询
    ...
    public void showRemaiDetail(String yourNumber) {
        ...
        if (map.get(yourNumber).setPackage instanceof TalkPackage) {
            ...
            remainSmsCount = cardPackage.smsCount > map.get(yourNumber).realSMSCount ? cardPackage.smsCount
                    - map.get(yourNumber).realSMSCount : 0;
            ...
            remainTalkTime = cardPackage.talkTime > map.get(yourNumber).realTalkTime ? cardPackage.talkTime
                    - map.get(yourNumber).realTalkTime : 0;
    ...
    public void printAmountDetail(String yourNumber) {
        ...
        if (isExist) {
            ...
            try {
                os = new FileWriter(yourNumber+"消费记录.txt");
        ...
        switch (optionrevie) {
            case 1:
                ...
                if (map.get(yourNumber).setPackage instanceof TalkPackage) {
                    ...
                    if (map.get(yourNumber).money >= sp.price) {
                        map.get(yourNumber).setPackage = sp;
                        map.get(yourNumber).realTalkTime = 0;
                        map.get(yourNumber).realFlow = 0;
                        map.get(yourNumber).realSMSCount = 0;
        ...
        if (map.containsKey(yourNumber)) {
            if (map.get(yourNumber).passWord.equals(yourPassWord)) {
                ...
                if (payMoney >= 50) {
                    ...
                    System.out.println("充值金额不能小于50元");
                }
